...
eta_list gen_eta(int nj,double niw_kap,vec sum_of_samples, vec Meta,
...
#define ZTOL (DOUBLE_EPS*10.0)
  ...
  long double log_sum_exp(vec x) 
  {
    uword max_ind=x.index_max();
  ...
  inline double log_t_density(double nu,  vec x,vec mu, mat lower_chol){
    int k=x.n_elem;
    double det_sig_half= sum(log(lower_chol.diag() ) );
    vec resid=solve(trimatl ( lower_chol ) , x - mu );
    
    double quad_form=as_scalar( crossprod(resid )); 
  ...
  inline double log_t_density_empty(double nu, double kap, double diag_psi_iw,mat adjustment_factor, vec x){
    int k=x.n_elem;
...
// [[Rcpp::export]]
umat DL_mixture( double dir_prec, double diag_psi_iw, double niw_kap,
  ...
  // --- Initiate PLAM --- //
  Plam= gsl_pow_2(T_sum)/(psi % square (T_phi*tau)) ;
  ...
  ///////////////////////////////////////////////////////////////////
  double alpha=dir_prec, prob_empty=log(dir_prec);
  ///////initialize parameters coresponding to clusters//////
  uvec nj_val(nmix);  //occupancu number corresponding to each cluster
  vec log_nj_val(nmix), niw_kap_post(nmix), df_post(nmix) ,niw_nu_post(nmix) ;
  mat  sum_of_samples=zeros<mat>(k,nmix),marginal_mu=zeros<mat>(k,nmix);
  cube sum_of_squares=zeros<cube>(k,k,nmix),   chol_marginal_delta=zeros<cube>(k,k,nmix),
    marginal_delta=zeros<cube>(k,k,nmix),tmpcube(k,k,nmix);
  inds_eq_j=new uvec[nmix ];
  double niw_nu=k+10; //wiki notation- prior DF for niw distn
  cout<<"cluster occupancy finding starts!!"<<endl;
  for(j=0;j<nmix;j++){
    ...
    if(dofactor){
      ...
      for(j=0;j<nmix;j++){
        if(nj_val(j)){
          ...
          // cout<<"Cholesky flag j="<<j<<" nj="<<nj<<endl;
          chol( chol_Delta.slice(j), Delta.slice(j) ); //chol_Delta is upper-triangular
          // det_of_cov_chol(j) =sum( log( diagvec ( chol_Delta.slice(j) ) ));
      ...
      // --- UPDATE LAMBDA --- //
      eta2 = eta.t() * eta;    // prepare eqta crossproduct before the loop
#pragma omp parallel for
      ...
      mat tau_prod_phi_div_abs_theta=(tau*T_phi)/  (T_sum* abs_theta) ;
      
      psi_inv = tau_prod_phi_div_abs_theta ;
      // cout<<"Neg psi indices before rig "<< find(psi<0).t()<<endl;
      // psi.transform(rig); psi=1/psi;
      psi_inv.transform( [](double val) { return ( rig(val) ); } );
      // psi=1/psi_inv;
      // cout<<"Neg phi indices "<< find(phi<0).t()<<endl;
      // cout<<"Neg psi indices "<< find(psi<0).t()<<endl;
      cout<<"psihj updated accu_psi_inv="<<accu(psi_inv)<< endl;
      if(psi.has_nan()){
        T_phi=tau * tau_prod_phi_div_abs_theta ;
        // T_phi.print("T_phi :");
        cout<<"corresponding T_phi variables to nan"<<  T_phi.elem( find_nonfinite( psi ) ) <<endl;
        cout<<"tau="<<tau<<endl;
        Rcpp::stop("psi has nan!!");
      }
      
      
      // --- UPDATE TAU --- //
      // vec chis = sum(Lambda_abs % pow(phi, -1), 1);
      // T_sum = tau*accu(1/tau_prod_phi_div_abs_theta);
      ...
      // --- UPDATE PHI --- //
      phi=abs_theta;
      phi.transform( [&a](double val) { double tmp=rgig(a-1, 2* val, 1); return(  GSL_MAX_DBL(tmp,1e-5)) ; } );
      double T_sum=accu(phi);
      T_phi=phi;
      phi/=T_sum;
      ...
      // --- UPDATE PLAM --- //
      // Plam=1+  psi % square (phi*tau) ;
      temp=gsl_pow_2(T_sum/tau);
      Plam= ( temp*(psi_inv/square(T_phi))); // square (T_phi*tau)/tmpmat;
      ...
      // vec log_det_approxmat_eta(nmix);
      //-----------------------------------------------------------//
      ...
      switch(approx){
      case 1:  adjustment_factor=inv_Lmsg2; //Noirrit's method
        break;
      case 2: adjustment_factor=2.0*inv_Lmsg2;//David's aproximation
        break;
      default: adjustment_factor=zeros<mat>(k,k);
      }
...
#pragma omp parallel for
      for(j=0;j<nmix;++j){
        ...
        if(nj_val(j)){
          ...
          chol_marginal_delta.slice(j)=chol(( t1ppp*marginal_delta.slice(j)+adjustment_factor), "lower");
          // cout<<"flag j= "<<j<<endl;
          tmpcube.slice(j)=chol_marginal_delta.slice(j);
        }
      }
    ...
    if(randu()<prob){
      ...
      for(int jj=0;jj<n;++jj){
        ...
        pdf_empty= log_t_density_empty((double) (niw_nu-k+1), niw_kap,diag_psi_iw,adjustment_factor, (eta.row(jj)).t() );
        ...
        for(unsigned j1=0;j1<nclust;++j1){
          ...
          if(j!=current_ind){
            if(nj_val(j)<1 ){
              ...
              dens= log_t_density(df_post(j) ,  (eta.row(jj)).t()
                                    ,marginal_mu.col(j), tmpcube.slice(j));
              // cout<<"at j="<<j<<" log_density="<<dens<<"\t clust prob="<<cluster_prob<<endl;
              cluster_prob=log_nj_val(j);
            }
            log_probs(j1)= ((i<burn) ? (dens/temper): dens) +cluster_prob;
          }
        }
        //calculating allocation probabilities
        ...
        if(nj_val(j)==1){
          ...
          t1ppp= multiplier_fn( niw_nu_post(j)-1 , niw_kap_post(j)-1, k);
          tmp_chol_marginal_delta=  chol(adjustment_factor+t1ppp* tmp_marginal_delta ,"lower") ;
          
          dens= log_t_density(df_post(j)-1 ,  (eta.row(jj)).t(),tmp_marginal_mu, tmp_chol_marginal_delta );
          cluster_prob=log( nj_val(j)-1 );
        }
        
        uvec indddd=find( non_empty_clusts==j);
        log_probs(indddd(0) )= ((i<burn) ? (dens/temper): dens) +cluster_prob;
        ...
        if(sum(probs)==0)  {
          ...
          // std::ofstream *debug_var_file, *debug_means_file;
          
          // debug_var_file=new std::ofstream[nmix];
          // debug_mean_file=new std::ofstream[nmix];
